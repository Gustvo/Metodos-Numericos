...
// f'(x) = 3x² - 9
constexpr double
func_derivada (double x)
{
  return 3 * x * x - 9;
}

// p(x) = x³ - 9x + 3
std::vector<double> polinomio = { 1, 0, -9, 3 };
...
getMethods ()
{
  ...
  functions.push_back (
      { "Secante", [=] () { return secante (ε, ε, 0, 1, func); } });

  functions.push_back ({ "Birge-Vieta", [=] () {
                          return birgeVieta (
                              iteracoesBirgeVieta, 1,
                              { polinomio.data (), polinomio.size () });
                        } });
...
static void
imprimeCabecalho (int numeroDeIteracoes)
{
  std::cout << "f(x) = x³ - 9x + 3\n"
            << "ε = " << ε
            << "\n\n"
               "\tNome\tx->f(x)=0\tIterações\tTempo (x"
            << numeroDeIteracoes << ")\n";
}

static void
imprimeNota (int repeticoes)
{
  std::cout << "\n"
...
static void
imprimeLinha (int numeroDeCasasDecimais, _resultados resultado)
{
...
static _resultados
calculaECronometraMetodo (Configuracao config, Metodo metodo)
{
...
static void
imprimeResultados (const std::vector<_resultados> tabela, Configuracao config)
{
...
void
calculaEImprimeExemplos (Configuracao config)
{
